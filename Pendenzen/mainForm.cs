...
namespace Pendenzen
{
    public partial class mainForm : Form
    {
        ...
        private string createAdressQuery(string searchKey, string id)
        {
            return
                "SELECT company_id, company_name, company_street, company_plz, company_city, company_country, company_verkaufmail, company_einkaufmail FROM company WHERE {searchKey} LIKE '%{id}%' AND company_verkaufkontakt = 'JA' OR company_einkaufkontakt = 'JA' ";
        }
        ...
        private string createQuery(string table)
        {
            ...
            printQuery = $"{baseQuery} FROM pendenz" + Query.admin("WHERE") + Query.department(departmentBox.Text) + Query.ending("desc");
            return $"{baseQuery} FROM {table}" + Query.admin("WHERE") + Query.department(departmentBox.Text) + Query.ending("desc");
        }
        ...
        private string createQuery(string table, string searchKey)
        {
            ...
            printQuery = $"{baseQuery} FROM pendenz WHERE {searchKey} = ''" + Query.admin() + Query.department(departmentBox.Text) + Query.ending("desc");
            return $"{baseQuery} FROM {table} WHERE {searchKey} = ''" + Query.admin() + Query.department(departmentBox.Text) + Query.ending("desc");
        }
        ...
        private string createQuery(string table, string searchKey, string searchText)
        {
            ...
            printQuery = baseQuery + $"FROM pendenz WHERE {searchKey} LIKE '%{searchText}%'" + Query.admin() + Query.department(departmentBox.Text) + Query.ending("desc");
            return baseQuery + $"FROM {table} WHERE {searchKey} LIKE '%{searchText}%'" + Query.admin() + Query.department(departmentBox.Text) + Query.ending("desc");
        }
        ...
        {
            String query = $"SELECT description FROM reason {Query.admin("WHERE")} {Query.department(departmentBox.Text)}";
            var dataTable = db.Select(query);
        ...
        private void loadStorno()
        {
            ...
            String query = $"SELECT * FROM stats {Query.admin("WHERE")} order by id desc";
            DataTable newTable = db.Select(query);
        ...
        private void getTopStornoComanies()
        {
            ...
            String query = $"SELECT company, COUNT(company) as c FROM stats {Query.department(departmentBox.Text, "WHERE")} GROUP BY company desc ORDER BY c DESC";
            ...
            query = $"SELECT company, SUM(amount) as c FROM stats {Query.department(departmentBox.Text, "WHERE")} GROUP BY company desc ORDER BY c DESC";
            ...
            query = $"SELECT reason, COUNT(reason) as c FROM stats {Query.department(departmentBox.Text, "WHERE")} GROUP BY reason desc ORDER BY c DESC";
            table = db.Select(query);
            foreach (DataRow row in table.Rows)
            {
            ...
            query = $"SELECT reason, SUM(amount) as c FROM stats {Query.department(departmentBox.Text, "WHERE")} GROUP BY reason ORDER BY c DESC";
            table = db.Select(query);
            foreach (DataRow row in table.Rows)
            {
        ...
        private void WriteToCSV()
        {
            if (isSearching)
            {
                ...
                query = createAdressQuery(searchKey, searchBox.Text);
            } else
            {
        ...
        {
            isSearching = false;
            query = baseQuery + "FROM pendenz WHERE state = 'open' " + Query.admin() + Query.department(departmentBox.Text) +" ORDER BY idpendenz desc";
            isOn = true;
            searchBox.Clear();
            searchDropBox.Text = "";
            ...
            if (searchDropBox.Text != "")
            {
                ...
                if (tabControl.SelectedIndex == 0)
                {
                    ...
                    if (searchStatusBox.Visible)
                        query = createQuery("pendenz", searchKey, searchStatusBox.Text) + Query.department(departmentBox.Text) + ending;
                    else if (searchBox.Visible && (searchBox.Text != ""))
                        query = createQuery("pendenz", searchKey, searchBox.Text) + Query.department(departmentBox.Text) + ending;
                    else if (searchBox.Visible && (searchBox.Text == ""))
                        query = createQuery("pendenz", searchKey) + Query.department(departmentBox.Text) + ending;
        ...
        public void departmentBox_SelectedIndexChanged(object sender, EventArgs e)
        {
            int index = departmentBox.SelectedIndex;
            query = query = baseQuery + "FROM pendenz WHERE state = 'open'" + Query.admin() + Query.department(departmentBox.Text) + Query.ending("desc");
            isOn = true;
        }
    }
}